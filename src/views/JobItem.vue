            ...
            <div class="flex flex-col md:flex-row md:items-center gap-3 md:justify-between ">
              <div class="flex flex-col md:flex-row items-center flex-1 gap-6">
                ...
                <div class="md:text-left">
                  ...
                  <div class="text-offgreen-dark text-lg mt-1">{{ data.company_name }} | {{ data.category }} | <span style="text-transform: capitalize;">{{ data.job_type.replace(/_/g, '-') }}</span></div>
                </div>
              </div>
              <div class="flex items-center flex-col md:first-letter:flex-row justify-between gap-px mr-2 md:flex-col md:items-end">
                <span class="text-xl font-medium">{{ data.candidate_required_location }}</span>
                <span class="text-2xl font-medium">{{ data.salary ? data.salary : 'Unspecified Salary' }}</span>
          ...
          <div class="py-8">
            ...
            <div class="flex gap-10 md:gap-5 flex-wrap items-start">
              <div v-for="tag in data.tags" :key="data.id" class=" shadow-lg flex flex-col">
                <span class="text-sm bg-offgreen-dark text-offgreen-light py-1 px-2 rounded border border-offgreen-dark">{{tag}}</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <form class="my-5">
            <div class="grid md:grid-cols-2 gap-6">
              <div>
                ...
                <input type="text" placeholder="First Name" autocomplete="false" class="w-full px-4 py-3 border-2 placeholder:text-offgreen-dark/50 rounded-md outline-none focus:ring-4 border-offgreen-medium focus:border-offgreen-dark ring-gray-100" name="first_name">
              </div>
              <div>
                <label class="text-base mb-2 block">Last Name</label>
                <input type="text" placeholder="Last Name" autocomplete="false" class="w-full px-4 py-3 border-2 placeholder:text-offgreen-dark/50 rounded-md outline-none focus:ring-4 border-offgreen-medium focus:border-offgreen-dark ring-gray-100" name="last_name">
              </div>
              <div>
                <label for="email_address" class="text-base mb-2 block">Email Address</label>
                <input id="email_address" type="email" placeholder="you@company.com" name="email" autocomplete="false" class="w-full px-4 py-3 border-2 placeholder:text-offgreen-dark/50 rounded-md outline-none focus:ring-4 border-offgreen-medium focus:border-offgreen-dark ring-gray-100">
              </div>
              <div>
                <label for="phone" class="text-base mb-2 block">Phone Number</label>
                <input id="phone" type="text" placeholder="Phone Number" autocomplete="false" class="w-full px-4 py-3 border-2 placeholder:text-offgreen-dark/50 rounded-md outline-none focus:ring-4 border-offgreen-medium focus:border-offgreen-dark ring-gray-100" name="phone">
              </div>
              <div>
                <label for="email_address" class="text-base mb-2 block">Address</label>
                <input type="text" placeholder="Full Address" autocomplete="false" class="w-full px-4 py-3 border-2 placeholder:text-offgreen-dark/50 rounded-md outline-none focus:ring-4 border-offgreen-medium focus:border-offgreen-dark ring-gray-100" name="address">
              </div>
              <div>
              ...
              <div class="md:col-span-2">
                ...
                <textarea placeholder="Cover Letter" class="w-full px-4 py-3 border-2 placeholder:text-offgreen-dark/50 rounded-md outline-none h-36 focus:ring-4 border-offgreen-medium focus:border-offgreen-dark ring-gray-100" name="cover"></textarea>
              </div>
            </div>
            <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-3 mt-5">
              <div>
                ...
                <input type="file" class="w-full mt-1" placeholder="Upload Resume" name="resume" accept="application/pdf,.doc,.docx">
              </div>
              <button type="submit" class="inline-flex  text-black bg-yellow-200 border border-yellow-200 rounded-full hover:border-black justify-center px-8 py-3">Apply Job</button>
            </div>
          </form>
        </div>
      </div>
...
onMounted(() => {
  ...
  if (job) {
    job.description = job.description.replace(/class='\"/g, 'class="')
                                     .replace(/'>/g, '">')
                                     .replace(/\"'/g, '"')
                                     .replace(/%22/g, '"');
    data.value = job
